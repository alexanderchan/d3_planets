...
var rPlanets = d3.scale.linear().range([0, height])

var xPlanets = d3.scale.linear().range([0, width])

var xAxisPlanets = d3.svg
  .axis()
  .orient('bottom')
  .scale(xPlanets)
...
export default class Maddie extends React.Component {
  ...
  planetStart(currentIndex) {
    return (
      this.state.planets.reduce((acc, planet, i) => {
        if (currentIndex >= i) {
          return acc
        } else {
  ...
  render() {
    return (
      <div className="content">
        ...
        <svg style={SVG_STYLE} onClick={this.addPlanet} width={width} height={height} ref={c => (this.chart = c)}></svg>
        <div>
          Based on <a href="https://www.youtube.com/watch?v=b-OHs70hZsM">this youtube vid</a> and some data from
          Wikipedia
        </div>
      </div>
    )
